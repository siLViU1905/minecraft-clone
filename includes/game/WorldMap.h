...
struct std::hash<glm::vec3>
{
    ...
    {
        return std::hash<int>()((int)pos.x) ^ std::hash<int>()((int)pos.y) ^ std::hash<int>()((int)pos.z);
    }
};
...
class WorldMap
{
    ...
    static void renderBlockMultiThreaded(WorldMap* map, int start, int end);

    struct UserEvents
    {
        //mouse left click
        bool mouseLeftClickThisFrame, mouseClickLastFrame, mouseLeftClick;
    ...
    std::pair<const Texture*,int> pickedBlock();

public:
void proccesUserEvents(GLFWwindow* window);

    WorldMap(const Camera* camera);
